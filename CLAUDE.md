...
- **Replik direkt efter cuen = `svar:{who,text}`** (sedan 23 sep 2026), inte `sub`: `svar` ritas i samma storlek som cuen, `sub` blir liten grå text. Richard ville ha 2:11 (”Tack”) och Olles ”Aaa!” lika stora.
...
- **Tempo och presets (sedan 16 sep 2026)** kommer ur `GUITARTECH/macken-axefx-presets.md`. `song` bär `bpm` (syns ALLTID – tempot är för hela bandet, även i snabböversiktens BPM-kolumn) och `preset`/`git`/`scen`/`tyst` (döljs med "Visa gitarr/preset"). `git` är en nyckel i `GIT` överst i arrayen (namn + Axe-FX-ingång), och kortets gitarrtaggar räknas ur låtarna – skriv dem inte i `tags`. `kind:"byte"` = gitarrbyte utan tyst preset, hela raden döljs med rutan. Aktpauserna bär `preset` + `byte`. Ändras presetfilen: uppdatera här i samma pass.
- **Richards egna inräkningar = `inr` på `song`-raden** (sedan 24 sep 2026): `inr:true` eller `inr:"2 takter …"` (texten hamnar i chipet). Ger blå ram, chipet "▶ RICHARD RÄKNAR IN", större BPM-ruta och ▶ + fylld siffra i översiktens BPM-kolumn. Låtar som någon annan räknar in rörs inte – de ska se ut exakt som förut (Richards önskemål). Sätt bara `inr` där han själv sagt att inräkningen är hans.
- **Paus MITT I ett kort = `{kind:"paus", text:"…"}` bland kortets items** (sedan 24 sep 2026, videofilmssketchen i 2:7). Ritas som pausraden men inne i kortet, och korten numreras inte om. En `{paus:…}` mellan korten hade i stället delat kortet och flyttat alla nummer efter det – gör det bara om Richard ber om det.
